...
We estimated IPTW using logistic regression with covariates $X_1$, $X_2$, and $X_3$. In separate weighting procedure, we used both logistic regression and Cox-Exponential regression with covariates $X_1$, $X_2$, and $X_3$ to estimate IPCW. All simulations and analyses were conducted in R. The logistic regressions were fitted using `glm` function and Cox-exponential model using `ipcw` from `riskRegression` R package.
...
bias_gg_1 <- 
  ...
  labs(x = "Scenario", y = "Relative Bias", col = "Weighting Procedure", title = "Relative Bias") +
...
coverage_gg <- 
  ...
  labs(x = "Scenario", y = "95% Coverage Rate", col = "Weighting Procedure", title = "95% Coverage Rates") +
...
It is apparent that there is improvement across all simulation settings in covariate balance in the IPTW$\circ$IPCW weighting method over the unweighted observed data in covariates $X_1$ and $X_2$, though this is not the case in $X_3$. We note that the covariate balance improved for most cases through the IPCW$\circ$IPTW weighting method over the unweighted observed data in $X_1$ and $X_2$, with the exclusion of the high-overlap settings, and see extreme imbalance in $X_3$. 
...
seq_weights_res %>% 
  ...
  kable_styling(latex_options = c("HOLD_position", "scale_down")) %>% 
  add_header_above(c(" " = 2, "IPTW$\\circ$IPCW" = 3, "IPCW$\\circ$IPTW"= 3), escape = FALSE) %>%
  add_footnote(c("Averaged over 1000 total bootstrapped replicates (100 bootstrap replicates over each of 100 samples)", "Note: scenarios are described by (degree of overlap/treatment effect level/censoring level)"))
```
...
(bias_gg_1 + coverage_gg) + plot_layout(guides = "collect") + plot_annotation(title = "Bias and 95% Coverage by Scenario and Weighting Procedure\nSequentials Weighting Procedures")
```
