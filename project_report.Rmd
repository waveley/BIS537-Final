...
library(progress)
n_1 <- 100000
pb <- progress_bar$new(total = n_1)

  u <- rnorm(n_1, 35)
  for(i in 1:n_1){
    cur_samp <- runif(500, 0, 2*u[i])
    t[i] <- mean(cur_samp)
  }
  t_hat <- mean(t)

ggplot(aes(x = x), data = tibble(x = t)) + 
...
run_true_expected_time <- function(n, params){
  ...
  for(i in 1:n){
    pb$tick()
    cur_samp <- runif(n)
    calculated_samp[i] <- (-log(cur_samp[i])/(lambda*exp(l[i])))^(1/v)
    t[i] <- mean(calculated_samp)
  }
...
t_1_true_val_list <- run_true_expected_time(n = 10000, params = list(1, 2, 1.5, 3, 0.0001, 3))
#t_1_true_val_list_2 <- run_true_expected_time(n = 500000, params = list(1, 2, 1.5, 3, 0.0001, 3))
#t_1_true_val_list_3 <- run_true_expected_time(n = 500000, params = list(1, 2, 1.5, 3, 0.0001, 3))
t_0_true_val_list <- run_true_expected_time(n = 10000, params = list(0, 2, 1.5, 3, 0.0001, 3))

g1 <- ggplot(aes(x = x), data = tibble(x = t_1_true_val_list[[1]])) + 
  geom_histogram(bins = 25)

#g2 <- ggplot(aes(x = x), data = tibble(x = t_1_true_val_list_2[[1]])) + 
  geom_histogram(bins = 25)

#g3 <- ggplot(aes(x = x), data = tibble(x = t_1_true_val_list_3[[1]])) + 
  geom_histogram(bins = 25)

t_1_true_val_list[[2]]
#t_1_true_val_list_2[[2]]
#t_1_true_val_list_3[[2]]
