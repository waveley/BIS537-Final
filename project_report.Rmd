---
title: "Comparison of Weighting Methods in the Estimation of Time-to-Event Causal Effects"
author: |
...
$$
\begin{aligned}
  \hat{\Delta}(t)&=\hat{S}_1(t)-\hat{S}_0(t)
  \\
...
\begin{aligned}
  ...
  \\
  \hat{\Delta}(5)&=\hat{S}_1(5)-\hat{S}_0(5)
  \\
...
table2_to_kable %>%
  ...
  knitr::kable(
    col.names = linebreak(c("Scenario$^\\text{b}$",
                  "Unweighted",
                  "Dual Logistic",
                  "Logistic/Cox",
                  "Unweighted",
      ...
      booktabs=TRUE,
    escape=FALSE,
     caption = "Separate Weighting Covariate Balance Results (ASD)$^\\text{a}$"
  )%>%
...
table1_to_kable %>%
  ...
  knitr::kable(
    col.names = c("Scenario$^\\text{b}$",
      ...
      booktabs=TRUE,
    escape=FALSE,
    caption = "Separate Weighting Estimation Results$^\\text{a}$"
  )%>%
  kable_styling(latex_options = c("HOLD_position", "scale_down")) %>% 
  add_header_above(c(" " = 2, "Dual Logistic" = 3, "Logistic/Cox"=3), escape = FALSE) %>%
   add_footnote(c("Obtained over 1000 total bootstrapped replicates (100 bootstrap replicates over each of 100 samples)", "Scenarios are described by (degree of overlap/treatment effect level/censoring level)"))
```
