...
Potential solutions to this dual-level missing data issue have been explored previously, with the construction of a combined weight using both treatment and censoring propensity scores (Hernan 2000, Chao 2022) 
...
The estimand of interest for this simulation study will be the restricted average survival causal effect at 5 months, defined as follows (Mao 2018):
...
This estimand is interpreted as the average difference in survival probabilities between the treatment and control groups, if both potential outcomes are observed, under the upper bound time restriction of $t^* = 5$.
...
The propensity score model will be defined using the following logistic regression: 
...
In our simulation, $\beta_1$, $\beta_2$, and $\beta_3$ will be varied to create settings of varying overlap (i.e., weak and strong overlaps), and $\beta_0$ will be determined through simulations to allow for the overall treatment proportion for all settings to be 55%. 
...
\tikzstyle{doublearr}=[latex-latex, black, line width=0.5pt]
  \begin{tikzpicture}[auto]
      ...
      \node [block, above right of = Z, xshift = 2cm, yshift = 1cm] (X1) {$X_1 \sim \text{Bernoulli}(0.6)$};
      \node [block, right of = X1, xshift = 2.5cm] (X2) {$X_2 \sim N(0,1)$};
      \node [block, right of = X2, xshift = 2.5cm] (X3) {$X_3 \sim \text{Gamma}(1,1)$};
      \node [block, below of = X2, yshift = -0.5cm] (T) {Survival Time};
      \node [block, right of = T, xshift = 5cm] (C) {Censoring Time};
      \node [block, below of = T, yshift = -1cm] (OT) {Observed Time};
      % Draw edges
      \path [line] (X1) -- (Z);
      \path [line] (X2) -- (Z);
      \path [line] (X2) -- (T);
