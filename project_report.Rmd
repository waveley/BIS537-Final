...
We have considered the utility of both indicator variables (treatment assignment and censoring indication) as weighting starting points because, under the assumption that censoring time and treatment assignment are independent of each other in this study, the relationship between censoring indication and treatment assignment is one in which temporality is difficult to evaluate. 
...
seq_weights_asd %>% 
  ...
  knitr::kable(
    ...
    booktabs=TRUE,
    escape=FALSE,
    caption = "Sequential Weighting Covariate Balance Results (ASD)"
  )%>%
  kable_styling(latex_options = c("HOLD_position", "scale_down")) %>% 
  add_header_above(c(" " = 1, "X1" = 3, "X2"= 3, "X3" = 3), escape = FALSE) %>%
  add_footnote(c("Note: scenarios are described by (degree of overlap/treatment effect level/censoring level)"))
