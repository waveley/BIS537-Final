...
We summarized the mean of estimated causal estimand, relative bias and $95\%$ coverage rate from the simulation for separate weighting procedure in Table 1. When using logistic regression to estimate both IPTW and IPCW, we observed smaller relative bias and higher $95\%$ coverage rate among low censoring scenarios regardless of overlap and treatment effect settings. The high overlap scenarios were also observed associated with higher $95\%$ coverage rate. However, such a pattern was not obvious when using Cox model to estimate IPCW. We also checked covaraites balancing by computing absolute standardized difference (ASD) and reported the results in table 2. Contrary to table 1, the performance of the method using logistic regression for IPTW and Cox model for IPCW was better than the model of dual logistic regression in terms of balancing baseline covariates.
...
table1_to_kable %>%
  ...
  janitor::clean_names() %>%
  knitr::kable(
    col.names = c("Scenario\n(Overlap/Treatment Effect/Censoring)",
                  "True ACE",
                  "Avg. Estimate",
                  "Relative Bias",
                  "95\\% Coverage Rate",
...
table2_to_kable %>%
  ...
  knitr::kable(
    col.names = c("Scenario\n(Overlap/Treatment Effect/Censoring)",
                  "Unweighted ASD",
                  "Dual Logistic Weighted ASD",
                  "Logistic/Cox Weighted ASD",
                  "Unweighted ASD",
                  "Dual Logistic Weighted ASD",
                  "Logistic/Cox Weighted ASD",
                  "Unweighted ASD",
...
seq_weights_res %>% 
  ...
  knitr::kable(
    col.names = c(
      "Scenario\n(Overlap/Treatment Effect/Censoring)", "True ACE",
      "Avg. Estimate",
      "Relative Bias",
      "95\\% Coverage Rate",
      "Avg. Estimate",
