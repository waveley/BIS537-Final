...
The remainder of this report is organized as follows: in section 2, we introduce our causal survival estimand; in section 3, we discuss our simulation design; in section 4, we illustrate our results from simulation study; and section 5 concludes with a discussion.
...
\begin{aligned}
  ...
  \\
  &=\Bigg(1-\frac{\sum_{i=1}^{n}\omega_{IPTW_i}A_i\delta_iI(U_i\leq t)\omega_{IPCW_i}}{\sum_{i=1}^{n}\omega_{IPTW_i}A_i}\Bigg)-\Bigg(1-\frac{\sum_{i=1}^{n}\omega_{IPTW_i}(1-A_i)\delta_iI(U_i\leq t)\omega_{IPCW_i}}{\sum_{i=1}^{n}\omega_{IPTW_i}(1-A_i)}\Bigg)
  \end{aligned}
$$

where $T_i^{S}$ is the event time of interest, $T_i^{C}$ is the censoring time, $U_i=min(T_i^{S}, T_i^{C})$ is the observed time in study, $\delta_i$ is the censoring indicator, $A_i$ is the treatment indicator, $I(U_i\leq t)$ is the indicator for the observed time being less than the upper limit of time $t$, and $\omega_{IPTW_i}$ and $\omega_{IPCW_i}$ are the inverse pIPTW and IPCW respectively. Then, our estimator would be defined as follows, at $t = 5$: 
$$
\begin{aligned}
  \hat{\Delta}(5)&= \Bigg(1-\frac{\sum_{i=1}^{n}\omega_{IPTW_i}A_i\delta_iI(U_i\leq 5)\omega_{IPCW_i}}{\sum_{i=1}^{n}\omega_{IPTW_i}A_i}\Bigg)
  -\Bigg(1-\frac{\sum_{i=1}^{n}\omega_{IPTW_i}(1-A_i)\delta_iI(U_i\leq 5)\omega_{IPCW_i}}{\sum_{i=1}^{n}\omega_{IPTW_i}(1-A_i)}\Bigg)
\end{aligned}
$$ 
...
\begin{aligned}
  \hat{\Delta}(t)&=\hat{S}(1)-\hat{S}(0)
  \\
  &=\Bigg(1-\frac{\sum_{i=1}^{n}\omega_{IPTW\circ IPCW_i}A_i\delta_iI(U_i\leq t)}{\sum_{i=1}^{n}\omega_{IPTW\circ IPCW_i}A_i}\Bigg)-\Bigg(1-\frac{\sum_{i=1}^{n}\omega_{IPTW\circ IPCW_i}(1-A_i)\delta_iI(U_i\leq t)}{\sum_{i=1}^{n}\omega_{IPTW\circ IPCW_i}(1-A_i)}\Bigg)
  \\
  \hat{\Delta}(5)&=\hat{S}(1)-\hat{S}(0)
  \\
  &=\Bigg(1-\frac{\sum_{i=1}^{n}\omega_{IPTW\circ IPCW_i}A_i\delta_iI(U_i\leq 5)}{\sum_{i=1}^{n}\omega_{IPTW\circ IPCW_i}A_i}\Bigg)-\Bigg(1-\frac{\sum_{i=1}^{n}\omega_{IPTW\circ IPCW_i}(1-A_i)\delta_iI(U_i\leq 5)}{\sum_{i=1}^{n}\omega_{IPTW\circ IPCW_i}(1-A_i)}\Bigg)
\end{aligned}
$$
...
## 2.3 Weighting Procedures (expand)

We estimated IPTW using logistic regression with covariates $X_1$, $X_2$, and $X_3$. In separate weighting procedure, we used both logistic regression and Exponential-Cox model with covariates $X_1$, $X_2$, and $X_3$ to estimate IPCW. All simulations and analyses were conducted in R. The logistic regressions were fitted using $glm()$ function and Cox model using $ipcw()$ from $RiskRegression$ R package.
...
As described in the previous diagram, we will be utilizing three covariates in this simulation study: $X_1$, $X_2$, $X_3$. These covariates have the following distributions: $X_1 \sim  \text{Bernoulli}(0.6)$, $X_2 \sim  N(0, 1)$, $X_3 \sim \text{Gamma}(1, 1)$ and will explicitly be defined to not be time dependent. 
...
\tikzstyle{doublearr}=[latex-latex, black, line width=0.5pt]
  \begin{tikzpicture}[auto]
      ...
      \node [block, text width = 3cm, right of = WD, xshift = 3cm] (EY) {$\hat{E}[S(1) - S(0)]$};
      % Draw edges
      \path [line] (D) -- (IPTW);
      \path [line] (D) -- (IPCW);
      \path [line] (IPTW) -- (WD);
...
We summarized the mean of estimated causal estimand, relative bias and $95\%$ coverage rate from the simulation for separate weighting procedure in Table 1. When using logistic regression to estimate both IPTW and IPCW, we observed smaller relative bias and higher $95\%$ coverage rate among low censoring scenarios regardless of overlap and treatment effect settings. The high overlap scenarios were also observed associated with higher $95\%$ coverage rate. However, such a pattern was not obvious when using Cox model to estimate IPCW. We also checked covariate balancing by computing absolute standardized difference (ASD) and reported the results in table 2. Contrary to table 1, the performance of the method using logistic regression for IPTW and Cox model for IPCW was better than the model of dual logistic regression in terms of balancing baseline covariates.
...
table2_to_kable %>%
  ...
  knitr::kable(
    col.names = linebreak(c("Scenario*",
                  "Unweighted\nASD",
                  "Dual Logistic\nWeighted ASD",
                  "Logistic/Cox\nWeighted ASD",
                  "Unweighted\nASD",
      ...
      booktabs=TRUE,
    escape=FALSE,
     caption = "Separate Weighting Covariate Balance Results"
  )%>%
  kable_styling(latex_options = c("HOLD_position", "scale_down")) %>% 
...
\tikzstyle{doublearr}=[latex-latex, black, line width=0.5pt]
  \begin{tikzpicture}[auto]
      ...
      \node [block, text width = 3cm, right of = WD2, xshift = 2.5cm] (EY) {$\hat{E}[S(1) - S(0)]$};
      ...
      \node [block, text width = 3cm, right of = WD2R, xshift = 2.5cm] (EYR) {$\hat{E}[S(1) - S(0)]$};
...
seq_weights_res %>% 
  ...
  knitr::kable(
    ...
    booktabs=TRUE,
    escape=FALSE,
    caption = "Sequential Weighting Estimation Results"
  )%>%
  kable_styling(latex_options = c("HOLD_position", "scale_down")) %>% 
